...
function renderContext(lan = "en") {
    ...
    $("#partTimeText").text(lan == "en" ? "Part-time Employment" : "파트타임 근무");
    $("#longTermText").text(lan == "en" ? "Long-term" : "장기");
    $("#shortTermText").text(lan == "en" ? "Short-term" : "단기");
    
    sections.forEach(function (section) {
        (section == "works_fulltime" ? works_fulltime : works_partLong).forEach(function (work) {
            var idVal = section == "works_fulltime" ? "#fulltime-id-" : "#parttime-long-id-";
            var parentRow = $(idVal + work.id);
            var downDiv = $("<div></div>").addClass("col-sm-8");
            var title = $("<h4></h4>").addClass("positionTitle").text(work.position[lan]);
            ...
            category.forEach(function(cate) {
                if (work[cate][lan] != "") {
                    ...
                    var dur = $("<p></p>").addClass("exp").text(work[cate][lan]);
            ...
            for (var i = 0; i < work.description[lan].length; ++i) {
                var _lst = $("<li></li>").addClass("bullet-points")
                    .text(work.description[lan][i]).css("text-align", "left");
                ul.append(_lst);
            }
            downDiv.append(ul);
    ...
    works_partShort.forEach(function(work) {
        ...
        if (work.id % 3 == 1) {
            ...
            var newRow = $("<div></div>").addClass("row").attr("id","row-id-" + row_num);
            parttimeShorttermContainer.append(newRow);
        }
        var outerContainer = $("<div></div>").addClass("shortterm-container").addClass("col-sm-4").addClass("col-xs-12");
...
function setImageInterval(imgContents, eachWork) {
    ...
    window.setInterval(function() {
        var index = eachWork.images.map(function(i) {
            return i.src;
        }).indexOf(imgItself.attr("src"));
        var len = eachWork.images.length;
        if (len == index + 1) index = -1;
        imgItself.fadeOut(function() {
            $(this).attr("src", eachWork.images[index + 1].src).fadeIn();
        });
        imgCred.fadeOut(function() {
            $(this).text(eachWork.images[index + 1].credit).fadeIn();
        });
    }, 8000); // 8s
}

fetch('json/work.json').then((response) => {
    ...
    sections.forEach(function (section) {
        (section == "works_fulltime" ? works_fulltime : works_partLong).forEach(function (work) {
            var imgRef = $("<p></p>").attr("id", "reference").text(work.images[0].credit);
            var imgCredit = $("<div></div>").addClass("image-credit");
            imgCredit.append(imgRef);
            
            var img = $("<img>").addClass("image-itself").attr("src", work.images[0].src).css("width","100%").attr("alt",work.position.en);
            ...
            var idVal = section == "works_fulltime" ? "fulltime-id-" : "parttime-long-id-";
            var parentRow = $("<div></div>").addClass("row").attr("id", idVal + work.id);
...
$("#lang-setting").on('change', function() {
    sections.forEach(function(section) {
        var idVal = section == "works_fulltime" ? "#fulltime-id-" : "#parttime-long-id-";
        (section == "works_fulltime" ? works_fulltime : works_partLong).forEach(function(work) {
            $(idVal + work.id).children().last().remove();
        });
    });
