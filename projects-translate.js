...
$("#lang-setting").on('change', function() {
    if ($(this).is(':checked')) { // From English to Korean
        ...
        $("#w1").text("개인 포트폴리오/웹사이트 호스팅 및 개설");
        ...
        $("#wf3").text("Google지도 API를 활용한 유저 위치 근처 식당 정보 제공");
        ...
        $("#s1").text("Backtracking 알고리즘을 사용한 효율적인 스도쿠 퍼즐 해결사 개발");
        ...
        $("#c1").text("Developed a Chess game with multiple modes using Waterfall SDLC collaboratively");
        ...
        $duration = $(b).text(dur);
        $(wfT + ".1").prepend($duration);
        $(wfT + ".2").text("Front-End Developer");
        $role = $(b).text(rol);
        $(wfT + ".2").prepend($role);
        $(wfT + ".3").text("HTML/CSS, Javascript");
        $language = $(b).text(lang);
